  ...
  public TodoSyncResponse sync(@RequestBody TodoSyncRequest sync,
    ...
    if (sync.getRemoved() != null && !sync.getRemoved().isEmpty()) {
      ...
      for (Long id : sync.getRemoved()) {
        int noOfDeleted = this.dsl.delete(TODO)
            .where(TODO.ID.eq(id).and(TODO.APP_USER_ID.eq(loggedInUserId))).execute();
        if (noOfDeleted == 1) {
          removed.add(id);
        }
      }
    ...
    if (sync.getUpdated() != null && !sync.getUpdated().isEmpty()) {
      ...
      for (Todo clientTodo : sync.getUpdated()) {
        var record = this.dsl
            .select(TODO.ID, TODO.UPDATED, TODO.SUBJECT, TODO.DESCRIPTION).from(TODO)
            .where(
                TODO.ID.eq(clientTodo.getId()).and(TODO.APP_USER_ID.eq(loggedInUserId)))
            .fetchOne();

        if (record != null) {
          if (record.get(TODO.UPDATED).toEpochSecond(ZoneOffset.UTC) > clientTodo
              .getTs()) {
            // db todo is newer than the version sent from the client. ignore client
            // update
            get.add(new Todo(record.get(TODO.ID),
                record.get(TODO.UPDATED).toEpochSecond(ZoneOffset.UTC),
            ...
            int noOfUpdated = this.dsl.update(TODO)
                ...
                .set(TODO.DESCRIPTION, clientTodo.getDescription()).set(TODO.UPDATED, now)
                .where(TODO.ID.eq(clientTodo.getId())
                    .and(TODO.APP_USER_ID.eq(loggedInUserId)))
                .execute();
            if (noOfUpdated == 1) {
    ...
    if (sync.getInserted() != null && !sync.getInserted().isEmpty()) {
      ...
      for (Todo clientTodo : sync.getInserted()) {
        ...
        long id = this.dsl
            .insertInto(TODO, TODO.SUBJECT, TODO.DESCRIPTION, TODO.UPDATED,
                TODO.APP_USER_ID)
            .values(clientTodo.getSubject(), clientTodo.getDescription(), now,
                loggedInUserId)
            .returning(TODO.ID).fetchOne().getId();
    ...
    if (sync.getGets() != null) {
      for (Long id : sync.getGets()) {
        var record = this.dsl
            .select(TODO.ID, TODO.UPDATED, TODO.SUBJECT, TODO.DESCRIPTION).from(TODO)
            .where(TODO.ID.eq(id).and(TODO.APP_USER_ID.eq(loggedInUserId))).fetchOne();
