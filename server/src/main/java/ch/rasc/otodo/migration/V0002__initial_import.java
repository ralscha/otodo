package ch.rasc.otodo.migration;
...
public class V0002__initial_import extends BaseJavaMigration {
	...
	public void migrate(Context context) {
		...
		dsl.transaction(txConf -> {
			...
			try {
				txdsl
					.insertInto(APP_USER, APP_USER.EMAIL, APP_USER.PASSWORD_HASH, APP_USER.AUTHORITY, APP_USER.ENABLED,
					...
					.values("admin@test.com", this.passwordEncoder.encode("password"), "ADMIN", true, null, null, null,
							null, null, null)
					.values("user@test.com", this.passwordEncoder.encode("password"), "USER", true, null, null, null,
							null, null, null)
					.execute();
			}
			catch (Exception e) {
