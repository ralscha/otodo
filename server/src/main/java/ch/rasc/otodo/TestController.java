...
class TestController {
	...
	public void disableUser() {
		this.dsl.update(APP_USER).set(APP_USER.ENABLED, false).where(APP_USER.EMAIL.eq("user@test.com")).execute();
	}
	...
	public void enableUser() {
		this.dsl.update(APP_USER).set(APP_USER.ENABLED, true).where(APP_USER.EMAIL.eq("user@test.com")).execute();
	}

	@GetMapping("/isEnabled")
	@PreAuthorize("hasAuthority('ADMIN')")
	public boolean isEnabled() {
		return this.dsl.select(APP_USER.ENABLED)
			.from(APP_USER)
			.where(APP_USER.EMAIL.eq("user@test.com"))
			.fetchOne()
			.value1();
	}
