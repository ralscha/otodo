...
class TodoServiceTest extends AbstractBaseTest {
  ...
  void testUserIsolation() {
    ...
    Todo newTodo1 = new Todo(-1, LocalDateTime.now().toEpochSecond(ZoneOffset.UTC),
        "subject1", "descr1");
    TodoSyncRequest sync1 = new TodoSyncRequest(List.of(newTodo1), null, null, null);

    Todo newTodo2 = new Todo(-1, LocalDateTime.now().toEpochSecond(ZoneOffset.UTC),
        "subject2", "descr2");
    ...
    var records1 = getDsl().selectFrom(TODO).where(TODO.APP_USER_ID.eq(user1.getId()))
        .fetchMap(TODO.ID);
    var records2 = getDsl().selectFrom(TODO).where(TODO.APP_USER_ID.eq(user2.getId()))
        .fetchMap(TODO.ID);
    ...
    int count = getDsl().selectCount().from(TODO)
        .where(TODO.APP_USER_ID.eq(user2.getId())).fetchOne(0, int.class);
    assertThat(count).isEqualTo(1);

    // try to update todo from other user
    Long todo2Id = records2.keySet().iterator().next();
    Todo updatedTodo = new Todo(todo2Id,
        LocalDateTime.now().toEpochSecond(ZoneOffset.UTC), "newSubject",
        "newDescription");
    TodoSyncRequest updateSync = new TodoSyncRequest(null, List.of(updatedTodo), null,
        null);
  ...
  void testUpdate() {
    ...
    TodoRecord todoRecord = getDsl().selectFrom(TODO)
        .where(TODO.APP_USER_ID.eq(user.getId())).fetchOne();
    ...
    Todo updatedTodo = new Todo(todoRecord.getId(), currentTs, "anotherNewSubject",
        "anotherNewDescription");
    TodoSyncRequest updateSync = new TodoSyncRequest(null, List.of(updatedTodo), null,
        null);

    ResponseEntity<TodoSyncResponse> response = sendSyncRequest(updateSync, token);

    TodoRecord updatedTodoRecord = getDsl().selectFrom(TODO)
        .where(TODO.APP_USER_ID.eq(user.getId())).fetchOne();
  ...
  void testRemove() {
    ...
    // remove
    TodoSyncRequest removeSync = new TodoSyncRequest(null, null,
        Set.of(todoRecord.getId()), null);
    ResponseEntity<TodoSyncResponse> response = sendSyncRequest(removeSync, token);

    TodoRecord record = getDsl().selectFrom(TODO).where(TODO.APP_USER_ID.eq(user.getId()))
        .fetchOne();
  ...
  private void compareTodos(List<Todo> userTodos, Map<Long, TodoRecord> dbRecords) {
    ...
    for (Todo todo : userTodos) {
      ...
      assertThat(dbRecord.getUpdated().toEpochSecond(ZoneOffset.UTC))
          .isEqualTo(todo.getTs());
    }
  }
  ...
  private void fetchSyncViewAndCompare(AppUserRecord user, String authToken) {
    ...
    var response = getRestTemplate().exchange("/be/syncview", HttpMethod.GET, request,
        typeRef);
    assertThat(response.getStatusCode().value()).isEqualTo(200);
    var sync = response.getBody();

    var records = getDsl().selectFrom(TODO).where(TODO.APP_USER_ID.eq(user.getId()))
        .fetch();

    assertThat(sync).hasSize(records.size());

    for (TodoRecord record : records) {
      assertThat(sync.get(record.getId()))
          .isEqualTo(record.getUpdated().toEpochSecond(ZoneOffset.UTC));
    }
  }
  ...
  private ResponseEntity<TodoSyncResponse> sendSyncRequest(TodoSyncRequest sync,
      String token) {
    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_JSON);
    headers.set(AuthHeaderFilter.HEADER_NAME, token);
    var request = new HttpEntity<>(sync, headers);
    ResponseEntity<TodoSyncResponse> response = getRestTemplate()
        .postForEntity("/be/sync", request, TodoSyncResponse.class);
    assertThat(response.getStatusCode().value()).isEqualTo(200);
    return response;
  }
