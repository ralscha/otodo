...
class SecurityTests extends AbstractEmailTest {
	...
	@ParameterizedTest
	@ValueSource(strings = { "/syncview", "/sync", "/change-password", "/delete-account", "/change-email", "/sessions",
			"/delete-session", "/invalidate-sessions", "/users", "/activate", "/delete", "/enable", "/disable" })
	...
	void testAuthenticate() {
		ResponseEntity<String> response = getRestTemplate().getForEntity("/be/authenticate", String.class);
		...
		response = getRestTemplate().exchange("/be/authenticate", HttpMethod.GET, request, String.class);
