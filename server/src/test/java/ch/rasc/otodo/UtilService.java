...
public class UtilService {
	...
	public String sendLogin(String username, String password, int expectStatusCode, String expectBody) {
		HttpHeaders headers = new HttpHeaders();
		...
		List<String> headerValues = response.getHeaders().getValuesAsList(AuthHeaderFilter.HEADER_NAME);
	...
	public long getUserId(String email) {
		return this.dsl.select(APP_USER.ID).from(APP_USER).where(APP_USER.EMAIL.eq(email)).fetchOne().get(APP_USER.ID);
	}
