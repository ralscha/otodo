...
class SignupTest extends AbstractEmailTest {
	...
	void signUpConfirmationWrongToken() {
		ResponseEntity<Boolean> response = getRestTemplate().postForEntity("/be/confirm-signup", "some_random_token",
				boolean.class);
	...
	@Test
	void signUpConfirmationAfterTokenMaxAge() throws MessagingException, IOException, FolderException {
		...
		getDsl().update(APP_USER)
			.set(APP_USER.CONFIRMATION_TOKEN_CREATED,
					appUser.getConfirmationTokenCreated().minus(getAppProperties().getSignupNotConfirmedUserMaxAge()))
	...
	void signUpAndDeleteAccount() throws FolderException, MessagingException, IOException {
		...
		String authToken = getUtilService().sendLogin(newUserEmail, "mypassword1234", 200, "USER");
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.TEXT_PLAIN);
		headers.set(AuthHeaderFilter.HEADER_NAME, authToken);

		// wrong password
		ResponseEntity<Boolean> deleteAccountResponse = getRestTemplate().postForEntity("/be/delete-account",
				new HttpEntity<>("wrongPassword", headers), boolean.class);
