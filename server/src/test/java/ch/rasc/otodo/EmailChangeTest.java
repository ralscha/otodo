...
class EmailChangeTest extends AbstractEmailTest {
	...
	void changeConfirmationWithWrongToken() {
		ResponseEntity<Boolean> response = getRestTemplate().postForEntity("/be/confirm-email-change", "unknown_token",
				boolean.class);
	...
	@Test
	void changeConfirmationAfterTokenMaxAge() throws MessagingException, IOException, FolderException {
	...
	void changeOkay() throws MessagingException, IOException, FolderException {
		...
		getDsl().update(APP_USER).set(APP_USER.EMAIL, oldEmail).where(APP_USER.EMAIL.eq(newEmail)).execute();
	}
	...
	private void sendChangeRequest(String authToken, String newEmail, String password, String expectedResponse) {
